...
def main() -> None:
    ...
    private_records = {record["target"]: record for record in private["records"]}
    require(private.get("prompt_hostname") == "hotdog", "wrong manifested shell prompt hostname")
    for prompt_path in ("system/etc/mkshrc", "system/etc/bash/bashrc"):
        ...
        require(
            ...
            and prompt_record.get("prompt_hostname") == "hotdog",
            f"shell prompt transformation is not audited: /{prompt_path}",
        )
    require_data(final_index, "file_contexts")
    mkshrc = require_data(final_index, "system/etc/mkshrc")
    bashrc = require_data(final_index, "system/etc/bash/bashrc")
    require(b": ${HOSTNAME:=hotdog}" in mkshrc, "Hotdog mksh prompt hostname is absent")
    require(b"export HOSTNAME=hotdog" in bashrc, "Hotdog Bash prompt hostname is absent")
    require(
    ...
    for marker, label in (
        ...
        (b"[OPLUS DECRYPT] no credential: selected direct-AES synthetic-password route", "passwordless direct-AES dispatcher"),
        (b"[OPLUS DECRYPT] no credential: selected empty-auth KeyStorage route", "passwordless empty-auth dispatcher"),
        (b"[OPLUS DECRYPT] no credential: deriving the default synthetic-password secret", "passwordless synthetic-password derivation"),
        (b"credential services unavailable before no-credential CE install", "passwordless Keystore2 readiness guard"),
        ...
        (b"I:[TWRP FORMAT] enforcing Android ext4 userdata features: %s", "ext4 userdata formatter marker"),
